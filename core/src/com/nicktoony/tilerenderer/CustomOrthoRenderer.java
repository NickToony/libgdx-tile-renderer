 ...
 * @author Nathan Sweet */
public class CustomOrthoRenderer implements TiledMapRenderer, Disposable {
    ...
    private SpriteCache currentCache = null;
    private int cacheCount = 0;
    private int cacheIndex = 0;
    private List<SpriteCache> caches;
    private List<Boolean> cacheUsed;

    /** @param cacheSize The maximum number of tiles that can be cached. */
    public CustomOrthoRenderer(TiledMap map, float unitScale, int cacheSize) {
        this.map = map;
        this.unitScale = unitScale;
        int count = 0;
        caches = new ArrayList<SpriteCache>();
        cacheUsed = new ArrayList<Boolean>();
        while (count < cacheSize) {
            caches.add(new SpriteCache(MAX_PER_BATCH, true));
            cacheUsed.add(false);
            count += MAX_PER_BATCH;
        }
    }
    ...
    public void render () {
        ...
        for (int c = 0; c < caches.size(); c ++) {
            SpriteCache spriteCache = caches.get(c);
            if (cacheUsed.get(c)) {
                spriteCache.begin();
    ...
    public void renderTileLayer (TiledMapTileLayer layer) {
        ...
        for (int row = row2; row >= row1; row--) {
            for (int col = col1; col < col2; col++) {
                ...
                currentCache.add(texture, vertices, 0, NUM_VERTICES);
                cacheUsed.set(cacheIndex, true);
                cacheCount += 1;
...
//        spriteCache.add(region.getTexture(), vertices, 0, NUM_VERTICES);
    ...
    @Override
    public void dispose () {
        for (SpriteCache spriteCache : caches) {
            spriteCache.dispose();
        }
    }
}
